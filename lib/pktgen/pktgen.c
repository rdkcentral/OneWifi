...
static int pktgen_configure(char *if_name, char *dest_mac, int packet_size)
{
    ...
    ret = pgctrl_command(PKTGEN_THREAD_FILE_0, "reset\n");
...
enum onewifi_pktgen_status onewifi_pktgen_start_wifi_blast(char *if_name, char *dest_mac, int packet_size)
{
    if (g_pktgen_blast_pid != 0) {
        ...
        return ONEWIFI_PKTGEN_STATUS_BUSY;
    }

    if (onewifi_pktgen_is_running()) {
    ...
    if (!pktgen_config_is_valid(if_name, dest_mac, packet_size)) {
        ...
        return ONEWIFI_PKTGEN_STATUS_FAILED;
    }
    ...
    return ONEWIFI_PKTGEN_STATUS_SUCCEED;
}
