...
int set_bus_callbackfunc_pointers(char *full_namespace, bus_callback_table_t *cb_table)
{
    bus_data_cb_func_t bus_data_cb[] = {
        ...
        { SECURITY_OBJ_TREE_NAME,
                            { security_get_param_value, security_set_param_value, NULL, NULL,
                security_event_sub_handler, NULL }                                                                },

        // wifi radius security event cb func
        { RADIUS_SEC_OBJ_TREE_NAME,
                            { radius_sec_get_param_value, radius_sec_set_param_value, NULL, NULL,
                radius_sec_event_sub_handler, NULL }                                                              },

        // wifi radius security event cb func
        { AUTH_SEC_OBJ_TREE_NAME,
                            { auth_sec_get_param_value, auth_sec_set_param_value, NULL, NULL,
                auth_sec_event_sub_handler, NULL }                                                                },
        ...
        { INTERWORKING_OBJ_TREE_NAME,
                            { interworking_get_param_value, interworking_set_param_value, NULL, NULL,
                interworking_event_sub_handler, NULL }                                                            },

        // wifi connection control event cb func
        { CONN_CTRL_OBJ_TREE_NAME,
                            { conn_ctrl_get_param_value, conn_ctrl_set_param_value, NULL, NULL,
                conn_ctrl_event_sub_handler, NULL }                                                               },

        // wifi pre connection control event cb func
        { PRE_CONN_CTRL_OBJ_TREE_NAME,
                            { pre_conn_ctrl_get_param_value, pre_conn_ctrl_set_param_value, NULL, NULL,
                pre_conn_ctrl_event_sub_handler, NULL }                                                           },

        // wifi post connection control event cb func
        { POST_CONN_CTRL_OBJ_TREE_NAME,
                            { post_conn_ctrl_get_param_value, post_conn_ctrl_set_param_value, NULL, NULL,
                post_conn_ctrl_event_sub_handler, NULL }                                                          },

        // wifi wps event cb func
        { WPS_OBJ_TREE_NAME,
                            { wps_get_param_value, wps_set_param_value, NULL, NULL, wps_event_sub_handler, NULL } },

        // wifi interworking serv event cb func
        { INTERWORKING_SERV_OBJ_NAME,
                            { interworking_serv_get_param_value, interworking_serv_set_param_value, NULL, NULL,
                interworking_serv_event_sub_handler, NULL }                                                       },

        // wifi passpoint event cb func
        { PASSPOINT_OBJ_TREE_NAME,
                            { passpoint_get_param_value, passpoint_set_param_value, NULL, NULL,
                passpoint_event_sub_handler, NULL }                                                               },

        // wifi client report event cb func
        { WIFI_CLIENT_REPORT_OBJ_NAME,
                            { wifi_client_report_get_param_value, wifi_client_report_set_param_value, NULL, NULL,
                wifi_client_report_event_sub_handler, NULL }                                                      },
        ...
        { NEIG_WIFI_DIAG_OBJ_NAME,
                            { neig_wifi_diag_get_param_value, neig_wifi_diag_set_param_value, NULL, NULL,
                neig_wifi_diag_event_sub_handler, NULL }                                                          },

        // wifi event cb func
        { NEIG_DIAG_RESULT_OBJ_NAME,
                            { neig_diag_result_get_param_value, NULL, neig_diag_result_table_add_row_cb,
                neig_diag_result_table_remove_row_cb, neig_diag_result_event_sub_cb, NULL }                       },

        // wifi ap macfilter event cb func
        { AP_MACFILTER_TREE_NAME,
                            { ap_macfilter_get_param_value, ap_macfilter_set_param_value, NULL, NULL,
                ap_macfilter_event_sub_handler, NULL }                                                            },

        // wifi region code event cb func
        { WIFI_REGION_OBJ_NAME,
                            { wifi_region_code_get_param_value, wifi_region_code_set_param_value, NULL, NULL, NULL,
                NULL }                                                                                            }
    };
...
int bus_register_namespace(bus_handle_t *handle, char *full_namespace,
    ...
    data_model_properties_t data_model_value, int num_of_rows)
{
    wifi_util_info_print(WIFI_DMCLI, "%s:%d: register_namespace:[%s] element_type:%d\n", __func__,
        __LINE__, full_namespace, element_type);
    ...
    if (element_type == bus_element_type_table) {
        ...
        wifi_util_info_print(WIFI_DMCLI, "%s:%d: Add number of row:%d input value:%d\n", __func__,
            __LINE__, dataElements.num_of_table_row, num_of_rows);
    ...
    bus_error_t rc = get_bus_descriptor()->bus_reg_data_element_fn(handle, &dataElements,
        num_elements);
...
int get_data_model_properties(cJSON *wifi_def_obj, char *data_str_type,
    data_model_properties_t *data_model_value)
{
    ...
    while (param != NULL) {
        if (!strncmp(param->string, "type", strlen("type"))) {
            ...
            data_model_value->str_validation = malloc(
    ...
    cJSON *wifi_def_obj, bus_handle_t *handle, bus_callback_table_t cb_table)
{
    ...
    for (uint32_t index = 0; index < num_of_elements; index++) {
        ...
        if (current_element->child != NULL) {
            ...
            if (strncmp(param->string, LIST_OF_DEFINITION_NAME, strlen(LIST_OF_DEFINITION_NAME)) ==
                0) {
                ...
                add_array_node_elem(param, sub_num_of_elem, full_namespace, wifi_def_obj, handle,
                    sub_elem_cb_table);
                continue;
            } else if (strncmp(param->string, MAX_NUM_OF_OBJECTS_NAME,
                ...
                bus_register_namespace(handle, full_namespace, temp_element_type, temp_cb_table,
                    data_model_value, l_num_of_tables);
                ...
                add_array_node_elem(param->next, sub_num_of_elem, full_namespace, wifi_def_obj,
                    handle, fetch_cb_table);
                continue;
            }
        }
        snprintf(full_namespace, sizeof(full_namespace), "%s.%s", name_prefix,
...
static int construct_namespace_and_register(bus_handle_t *handle, cJSON *cfg_param,
    cJSON *wifi_def_obj, char *l_name_prefix)
{
    ...
    if (cJSON_IsObject(cfg_param) == true) {
        ...
        while (current_element != NULL) {
            ...
            if (tmp->string != NULL &&
                strncmp(tmp->string, LIST_OF_DEFINITION_NAME, strlen(LIST_OF_DEFINITION_NAME)) ==
                    0) {
                construct_namespace_and_register(handle, tmp, wifi_def_obj, full_namespace);
            } else if (tmp->string != NULL &&
                strncmp(tmp->string, MAX_NUM_OF_OBJECTS_NAME, strlen(MAX_NUM_OF_OBJECTS_NAME)) ==
                    0) {
                ...
                decode_json_param_bool(current_element, "writable", param);
                data_model_value.data_permission = (param->type & cJSON_True) ? true : false;
                bus_register_namespace(handle, full_namespace, element_type, cb_table,
                    data_model_value, 0);
            }
        ...
        memset(&data_model_value, 0, sizeof(data_model_value));
        // main table register
        bus_register_namespace(handle, name_prefix, element_type, temp_cb_table, data_model_value,
            num_of_tables);
        ...
        for (int index = 0; index < num_of_elements; index++) {
            ...
            if (cJSON_IsObject(current_element) == true) {
                construct_namespace_and_register(handle, current_element, wifi_def_obj,
                    name_prefix);
            } else {
                uint32_t temp_num_of_elements = cJSON_GetArraySize(cfg_param);
                add_array_node_elem(cfg_param, temp_num_of_elements, name_prefix, wifi_def_obj,
                    handle, cb_table);
            }
        }
    } else {
...
static void decode_wifi_object_recurse(bus_handle_t *handle, cJSON *node, cJSON *wifi_def_obj,
    char *l_name_prefix)
{
    VERIFY_NULL(node);
