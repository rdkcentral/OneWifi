...
int get_sec_modes_supported(int vap_index, int *mode)
{
    ...
    if (band == WIFI_FREQUENCY_6_BAND) {
        *mode = passpoint_enabled ? wifi_security_mode_wpa3_enterprise :
                ...
                wifi_security_mode_wpa3_enterprise | wifi_security_mode_enhanced_open;
    ...
    for (index = 0; index < (uint32_t)ARRAY_SZ(wifi_chan_width_map); index++) {
        if (l_chan_width == wifi_chan_width_map[index].hal_wifi_chan_width) {
            ...
            wifi_util_dbg_print(WIFI_DMCLI, "%s:%d inputBw: %d str bw:[%s]\n", __func__, __LINE__,
                l_chan_width, wifi_chan_width_map[index].str_wifi_chan_width);
            str_found = true;
            break;
        }
    }
...
int radio_wifi_channel_is_valid(uint32_t radio_index, uint32_t input_channel)
{
    ...
    if (p_radio_cap == NULL) {
        wifi_util_error_print(WIFI_DMCLI,
            "%s Failed for unable to get RadioCapability wlan_Index=%d\n", __func__, radio_index);
        return RETURN_ERR;
    }
    ...
    for (band_arr_index = 0; band_arr_index < p_radio_cap->numSupportedFreqBand; band_arr_index++) {
        if (p_radio_cap->band[band_arr_index] == p_radio_cfg->band) {
            wifi_util_info_print(WIFI_DMCLI, "%s Band = %d is present at array index of cap:%d\n",
                __func__, p_radio_cfg->band, band_arr_index);
            is_band_found = true;
            break;
        }
    }
    ...
    if (is_band_found == false) {
        wifi_util_error_print(WIFI_DMCLI, "%s Input radio_index=%d Band=%d is not found in cap\n",
            __func__, radio_index, p_radio_cfg->band);
        return RETURN_ERR;
    }
    ...
    for (seq_counter = 0; seq_counter < arr_len; seq_counter++) {
        if (input_channel ==
            (uint32_t)p_radio_cap->channel_list[band_arr_index].channels_list[seq_counter]) {
            wifi_util_info_print(WIFI_DMCLI,
                "%s %d For radio_index:%d input_channel:%d is supported\n", __func__, __LINE__,
                radio_index, input_channel);
            return RETURN_OK;
        }
    }
    wifi_util_error_print(WIFI_DMCLI, "%s Failed for radio_index:%d for input_channel:%d\n",
        __func__, radio_index, input_channel);
    return RETURN_ERR;
}
...
bool is_radio_tx_power_valid(char *p_supported_power_list, int l_tx_power)
{
    ...
    wifi_util_error_print(WIFI_DMCLI,
        "%s:%d Given Tx power value:%d is not supported and supported values:%s\n", __func__,
    ...
    wifi_operating_env_t l_oper_env, char *str_reg_domain)
{
    ...
    for (i = 0; i < (uint32_t)ARRAY_SZ(wifiCountryMapMembers); ++i) {
        if (l_country_code == wifiCountryMapMembers[i].countryCode) {
            strncpy(tmp_country_str, wifiCountryMapMembers[i].countryStr,
                sizeof(tmp_country_str) - 1);
            break;
        }
    }
    ...
    for (i = 0; i < (uint32_t)ARRAY_SZ(wifiEnviromentMap); ++i) {
        if (l_oper_env == wifiEnviromentMap[i].operatingEnvironment) {
            strncpy(tmp_environment, wifiEnviromentMap[i].environment,
                sizeof(wifiEnviromentMap[i].environment) - 1);
            break;
        }
    }
    ...
    wifi_countrycode_type_t *p_country_code, wifi_operating_env_t *p_oper_env)
{
    ...
    if (environment[0] == '\0') {
        environment[0] = ' ';
    } else if (environment[0] != 'I' && environment[0] != 'O' && environment[0] != ' ' &&
        environment[0] != 'X') {
...
{
    push_event_to_ctrl_queue(webconfstr, size, wifi_event_type_webconfig,
...
{
    const char *exist_dbg_log_str[] = { "wifiDbDbg", "wifiMgrDbg", "wifiWebConfigDbg",
        "wifiCtrlDbg", "wifiPasspointDbg", "wifiDppDbg", "wifiMonDbg", "wifiDMCLI", "wifiLib",
        "wifiPsm", "wifiLibhostapDbg", "wifiHalDbg" };
    uint32_t index;
    bool str_found = false;
    ...
    while (token != NULL) {
        for (index = 0; index < num_strings; index++) {
            if (STR_CMP(token, exist_dbg_log_str[index])) {
                ...
                snprintf(temp_dbg_log_name, sizeof(temp_dbg_log_name), "/nvram/%s",
                    exist_dbg_log_str[index]);
    ...
    if (mac[2] == ':' && mac[5] == ':' && mac[8] == ':' && mac[11] == ':' && mac[14] == ':') {
        for (iter = 0; iter < MAX_STR_MAC_ADDR_LEN; iter++) {
            if ((iter == 2 || iter == 5 || iter == 8 || iter == 11 || iter == 14)) {
                continue;
            } else if ((mac[iter] > 47 && mac[iter] <= 57) || (mac[iter] > 64 && mac[iter] < 71) ||
...
assoc_dev_data_t *get_sta_assoc_data_map(uint32_t ap_index, uint32_t sta_index)
{
    ...
    if (p_assoc_sta_map == NULL) {
        wifi_util_error_print(WIFI_DMCLI, "%s:%d assoc sta is NULL pointer:%d\n", __func__,
            __LINE__, vap_param->vap_index);
        pthread_mutex_unlock(&((webconfig_dml_t *)get_webconfig_dml())->assoc_dev_lock);
        return NULL;
    }

    hash_element_t *hash_ele_queue_data = (hash_element_t *)queue_peek(p_assoc_sta_map->queue,
        sta_index - 1);
...
{
    uint32_t index;
    unsigned long inst_client_reproting_periods[] = { 0, 1, 5, 15, 30, 60, 300, 900, 1800, 3600,
...
int is_sec_mode_open_for_private_ap(uint32_t vap_index)
{
    ...
    if (p_radio_oper_param == NULL) {
        wifi_util_error_print(WIFI_DMCLI,
            "%s Input radioIndex = %d not found for radio_oper_param\n", __func__,
    ...
    if ((p_radio_oper_param->band == WIFI_FREQUENCY_6_BAND)) {
        wifi_util_error_print(WIFI_DMCLI,
            "%s Input radioIndex = %d with 6G Band doesnot support WPS:%d\n", __func__,
    ...
    if (p_vap_info->u.bss_info.security.mode == wifi_security_mode_none) {
        wifi_util_error_print(WIFI_DMCLI,
            "%s Open Security for vap_index : %d, WPS doesnot support \n", __func__, vap_index);
        return RETURN_ERR;
    }
    ...
    if ((p_vap_info->u.bss_info.security.mode == wifi_security_mode_wpa3_personal) ||
        ...
        wifi_util_error_print(WIFI_DMCLI,
            "%s Input radioIndex = %d WPS doesnot support WPA3 Mode:%d\n", __func__,
            p_vap_info->radio_index, p_vap_info->u.bss_info.security.mode);
        return RETURN_ERR;
    }
...
acl_entry_t *get_macfilter_entry(wifi_vap_info_t *vap_info, uint32_t acl_entry_index)
{
    ...
    wifi_util_dbg_print(WIFI_DMCLI,
        "%s:%d vap_index:%d acl_entry_index:%d"
        " count_hash:%d count_queue:%d\n",
        __func__, __LINE__, vap_info->vap_index, acl_entry_index, count_hash, count_queue);
...
static int sync_bus_macfilter_table_vap_entries(bus_handle_t *handle, wifi_vap_info_t *vap_param)
{
    ...
    wifi_util_info_print(WIFI_DMCLI, "%s:%d max macfilter cnt:%d for vap:%d, dml table_cnt:%d\n",
        __func__, __LINE__, max_acl_cnt, vap_param->vap_index,
    ...
    if (p_dml_param->table_macfilter_index[vap_param->vap_index] != max_acl_cnt) {
        ...
        snprintf(buff, sizeof(buff), "Device.WiFi.AccessPoint.%d.X_CISCO_COM_MacFilterTable.",
            vap_param->vap_index + 1);
        if (p_dml_param->table_macfilter_index[vap_param->vap_index] < max_acl_cnt) {
            index = p_dml_param->table_macfilter_index[vap_param->vap_index] + 1;
            while (index <= max_acl_cnt) {
                if (p_bus_desc->bus_reg_table_row_fn(handle, buff, index, NULL) !=
                    bus_error_success) {
                    wifi_util_error_print(WIFI_DMCLI,
                        "%s:%d macfilter table_reg row:%s failed for index:%d\n", __func__,
                        __LINE__, buff, index);
                } else {
                    p_dml_param->table_macfilter_index[vap_param->vap_index]++;
                }
            ...
            while (index > max_acl_cnt) {
                ...
                if (p_bus_desc->bus_remove_table_row_fn(handle, row_rem_buff) !=
                    bus_error_success) {
                    wifi_util_error_print(WIFI_DMCLI,
                        "%s:%d macfilter table remove row:%s failed\n", __func__, __LINE__,
                        row_rem_buff);
                }
                index--;
            }
...
static int sync_dml_macfilter_index(void *arg)
{
    ...
    sscanf(input_arg->table_row, "Device.WiFi.AccessPoint.%d.X_CISCO_COM_MacFilterTable.%d",
        &vap_index, &macfilter_entry_index);
    snprintf(input_arg->table_row, sizeof(input_arg->table_row),
        "Device.WiFi.AccessPoint.%d.X_CISCO_COM_MacFilterTable.", vap_index);
    snprintf(new_row_name, sizeof(new_row_name), "%s%d", input_arg->table_row,
        *cur_macfilter_index);

    wifi_util_info_print(WIFI_DMCLI,
        "%s:%d table add:%s = remove:%s for vap:%d remove index:%d add back index:%d \n", __func__,
        __LINE__, input_arg->table_row, new_row_name, input_arg->vap_index, *cur_macfilter_index,
        macfilter_entry_index);

    if (p_bus_desc->bus_reg_table_row_fn(&ctrl->handle, input_arg->table_row, macfilter_entry_index,
            NULL) != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI, "%s:%d table_reg row:%s failed for index:%d\n", __func__,
            __LINE__, input_arg->table_row, macfilter_entry_index);
    } else {
...
int sync_dml_macfilter_table(uint32_t vap_index, char *table_row_name)
{
    ...
    wifi_util_info_print(WIFI_DMCLI, "%s:%d macfilter table:%s sync start for vap:%d\n", __func__,
        __LINE__, table_row_name, vap_index);
    scheduler_add_timer_task(ctrl->sched, FALSE, NULL, sync_dml_macfilter_index, input_arg,
        (MACFILTER_TABLE_SYNC_TIME_IN_SEC * 1000), 1, FALSE);

    return RETURN_OK;
}

static int sync_bus_sta_assoc_table_vap_entries(bus_handle_t *handle, uint32_t vap_index,
    uint32_t max_sta_count)
{
    wifi_dml_data_model_t *p_dml_param = get_dml_data_model_param();

    wifi_util_info_print(WIFI_DMCLI, "%s:%d max sta assoc cnt:%d for vap:%d, dml table_cnt:%d\n",
        __func__, __LINE__, max_sta_count, vap_index,
        ...
        if (p_dml_param->table_sta_assoc_index[vap_index] < max_sta_count) {
            ...
            while (index <= max_sta_count) {
                if (p_bus_desc->bus_reg_table_row_fn(handle, buff, index, NULL) !=
                    bus_error_success) {
                    wifi_util_error_print(WIFI_DMCLI,
                        "%s:%d sta assoc table_reg row:%s failed for index:%d\n", __func__,
                        __LINE__, buff, index);
                } else {
                    p_dml_param->table_sta_assoc_index[vap_index]++;
                }
            ...
            while (index > max_sta_count) {
                ...
                if (p_bus_desc->bus_unreg_table_row_fn(handle, row_rem_buff) != bus_error_success) {
                    wifi_util_error_print(WIFI_DMCLI,
                        "%s:%d sta assoc table remove row:%s failed\n", __func__, __LINE__,
                        row_rem_buff);
                } else {
                    p_dml_param->table_sta_assoc_index[vap_index]--;
                }
...
int get_dml_total_associated_devices_count(uint32_t radio_index, uint32_t vap_array_index,
    uint32_t *count)
{
    int ret = RETURN_ERR;
...
int sync_dml_sta_assoc_table_entries(void)
{
    ...
    for (r_index = 0; r_index < get_num_radio_dml(); r_index++) {
        ...
        for (v_index = 0; v_index < get_max_num_vaps_per_radio_dml(r_index); v_index++) {
            if (get_dml_total_associated_devices_count(r_index, v_index, &max_sta_count) ==
                RETURN_OK) {
...
#define CPE_MAC_NAMESPACE "Device.X_CISCO_COM_CableModem.MACAddress"
    ...
    if (strlen(device_mac) == 0) {
        ...
        if (rc != bus_error_success || (data.data_type != bus_data_type_string)) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d bus_data_get_fn failed for [%s]"
                " with error [%d]\n",
    ...
    if (data == NULL) {
        ...
        if (!json) {
            ...
            if (NULL != partner_obj) {
                ...
                if (NULL != param_obj) {
                    cJSON_ReplaceItemInObject(param_obj, "ActiveValue",
                        cJSON_CreateString(p_value));
                    cJSON_ReplaceItemInObject(param_obj, "UpdateTime",
                        cJSON_CreateString(p_current_time));
                    cJSON_ReplaceItemInObject(param_obj, "UpdateSource",
                        cJSON_CreateString(p_source));
                    ...
                    if (!config_update_status) {
                        wifi_util_error_print(WIFI_DMCLI,
                            "Bootstrap config update: %s, %s, %s, %s \n", p_key, p_value,
                            partner_id, p_source);
                    } else {
                        wifi_util_error_print(WIFI_DMCLI, "Failed to update value for %s partner\n",
                            partner_id);
