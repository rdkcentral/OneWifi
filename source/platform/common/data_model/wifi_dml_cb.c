    ...
    if (STR_CMP(param_name, "ApplyRadioSettings")) {
        ...
        if (output_value && p_rfc_cfg->radiusgreylist_rfc) {
            wifi_util_dbg_print(WIFI_DMCLI,
                "%s:%d:RadiussGreyList enabled=%d hence cannot enable preferPrivate\n", __func__,
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:rapid_reconnect_enable=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.rapid_reconnect_enable, output_value);
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:mfp_config_feature=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.mfp_config_feature, output_value);
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:tx_overflow_selfheal=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.tx_overflow_selfheal, output_value);
        ...
        for (instance_number = 0; instance_number < (uint32_t)getNumberRadios();
            ...
            if (dm_wifi_radio_op_param == NULL) {
                wifi_util_error_print(WIFI_DMCLI,
                    "%s:%d Unable to get Radio Param for instance_number:%d\n", __func__, __LINE__,
                    instance_number);
                return false;
            }
            if (output_value) {
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:force_disable_radio_status=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.force_disable_radio_feature,
            output_value);
        ...
        wifi_util_dbg_print(WIFI_DMCLI,
            "%s:%d push_managed_wifi_disable_to_ctrl_queue to ctrl queue\n", __func__, __LINE__);
        if (!output_value) {
            if (push_managed_wifi_disable_to_ctrl_queue() == RETURN_ERR) {
                wifi_util_error_print(WIFI_DMCLI,
                    "%s:%d Failed to push_managed_wifi_disable_to_ctrl_queue to ctrl queue\n",
                    __func__, __LINE__);
                return false;
            }
        } else {
        ...
        if (output_value && p_global_wifi_cfg->global_parameters.prefer_private) {
            wifi_util_dbg_print(WIFI_DMCLI,
                "prefer_private is set to false when radiusgreylist is enabled\n");
        ...
        if (output_value != p_rfc_cfg->wifi_offchannelscan_sm_rfc) {
            push_rfc_dml_cache_to_one_wifidb(output_value,
...
bool wifi_set_param_int_value(void *obj_ins_context, char *param_name, int output_value)
{
    ...
    if (STR_CMP(param_name, "X_RDKCENTRAL-COM_GoodRssiThreshold")) {
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:good_rssi_threshold=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.good_rssi_threshold, output_value);
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:assoc_count_threshold=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.assoc_count_threshold, output_value);
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:assoc_monitor_duration=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.assoc_monitor_duration, output_value);
        ...
        p_global_wifi_cfg->global_parameters.whix_log_interval =
            output_value; // update global structure
        if (push_global_config_dml_cache_to_one_wifidb() != RETURN_OK) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: Failed to push WHIX_LogInterval to onewifi db\n", __func__, __LINE__);
        }
    } else if (STR_CMP(param_name, "WHIX_ChUtility_LogInterval")) {
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d: WHIX_chutility_LogInterval=%d Value=%d\n", __func__,
            __LINE__, p_global_wifi_cfg->global_parameters.whix_chutility_loginterval,
            output_value);
        p_global_wifi_cfg->global_parameters.whix_chutility_loginterval =
            output_value; // update global structure
        if (push_global_config_dml_cache_to_one_wifidb() != RETURN_OK) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: Failed to push WHIX_LogInterval to onewifi db\n", __func__, __LINE__);
...
bool radio_get_param_bool_value(void *obj_ins_context, char *param_name, bool *output_value)
{
    ...
    if (STR_CMP(param_name, "Enable")) {
        ...
        *output_value = dm_radio_default->APIsolation;
        ...
        wifi_radio_capabilities_t radio_capab =
            ((webconfig_dml_t *)get_webconfig_dml())->hal_cap.wifi_prop.radiocap[radio_index];

        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d X_COMCAST_COM_DFSSupport band %d num_channels %d\n",
            __func__, __LINE__, pcfg->band, radio_capab.channel_list[0].num_channels);
        for (int i = 0; i < radio_capab.channel_list[0].num_channels; i++) {
            if ((pcfg->band == WIFI_FREQUENCY_5_BAND || pcfg->band == WIFI_FREQUENCY_5L_BAND ||
                    pcfg->band == WIFI_FREQUENCY_5H_BAND) &&
                (radio_capab.channel_list[0].channels_list[i] >= 52 &&
                    radio_capab.channel_list[0].channels_list[i] <= 144)) {
                *output_value = true;
                return true;
            }
        }
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "Name")) {
        ...
        STR_CMP(param_name, "SupportedFrequencyBands")) {
        /* collect value */
        char buff[16] = { 0 };
        ...
        radio_cap =
            &((webconfig_dml_t *)get_webconfig_dml())->hal_cap.wifi_prop.radiocap[radio_index];
        if (get_allowed_channels_str(pcfg->band, radio_cap, buff, sizeof(buff),
                rfc_params->dfs_rfc) != RETURN_OK) {
        ...
        if (get_reg_domain_string_from_int(pcfg->countryCode, pcfg->operatingEnvironment,
                reg_domain_str) == 0) {
            wifi_util_error_print(WIFI_DMCLI, "%s:%d: get regulatory domain is failed:%d\n",
                __func__, __LINE__, pcfg->countryCode);
            return false;
        }
...
bool radio_set_param_bool_value(void *obj_ins_context, char *param_name, bool output_value)
{
    ...
    if (STR_CMP(param_name, "Enable")) {
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d: enable=%d set value=%d radio_index=%d\n", __func__,
            __LINE__, dm_radio_param->enable, output_value, radio_index);
        if (dm_wifi_global_cfg->global_parameters.force_disable_radio_feature) {
            wifi_util_dbg_print(WIFI_DMCLI,
                "%s:%d:WIFI_ATTEMPT_TO_CHANGE_CONFIG_WHEN_FORCE_DISABLED\n", __func__, __LINE__);
            return false;
        }
        if (get_radio_presence(&((webconfig_dml_t *)get_webconfig_dml())->hal_cap.wifi_prop,
                radio_index) == false) {
            wifi_util_dbg_print(WIFI_DMCLI,
                "%s:%d: Not allowed to change config when radio is not present in CPE \n", __func__,
                __LINE__);
            return false;
        }
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d: RadioEnable : %d\n", __func__, __LINE__,
            dm_radio_param->enable);
    } else if (STR_CMP(param_name, "AutoChannelEnable")) {
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:autoChannelEnabled=%d set value=%d\n", __func__,
            __LINE__, dm_radio_param->autoChannelEnabled, output_value);
        ...
        wifi_rfc_dml_parameters_t *rfc_pcfg =
            (wifi_rfc_dml_parameters_t *)get_wifi_db_rfc_parameters();
        if (!(rfc_pcfg->dfs_rfc)) {
        ...
        wifi_util_dbg_print(WIFI_DMCLI,
            "%s:%d: Updated radio enable status based on EcoPowerDown,=%d, Enable=%d\n", __func__,
...
bool radio_set_param_int_value(void *obj_ins_context, char *param_name, int output_value)
{
    ...
    if (STR_CMP(param_name, "MCS")) {
        ...
        if (dm_radio_param->transmitPower == (uint32_t)output_value) {
            return true;
        } else if (is_radio_tx_power_valid(dm_radio_default->TransmitPowerSupported,
                       output_value) != true) {
            return false;
        }
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:transmitPower=%d radio_index=%d\n", __func__,
            __LINE__, dm_radio_param->transmitPower, radio_index);
    } else if (STR_CMP(param_name, "X_CISCO_COM_MbssUserControl")) {
...
bool radio_set_param_uint_value(void *obj_ins_context, char *param_name, uint32_t output_value)
{
    ...
    if (STR_CMP(param_name, "Channel")) {
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s fragmentationThreshold:%d\n", __func__,
            dm_radio_param->fragmentationThreshold);
        is_radio_config_changed = true;
    } else if (STR_CMP(param_name, "X_CISCO_COM_DTIMInterval")) {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "OperatingStandards")) {
        ...
        if ((pcfg->band == WIFI_FREQUENCY_2_4_BAND) && (rfc_pcfg->twoG80211axEnable_rfc == false) &&
            (strstr(output_value->buff, "ax") != NULL)) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: wifi hw variant:%s radio_band:%d 80211axEnable rfc:%d\n", __func__,
                __LINE__, output_value->buff, pcfg->band, rfc_pcfg->twoG80211axEnable_rfc);
            return false;
        }
        ...
        if (validate_wifi_hw_variant(pcfg->band, radio_variant) != RETURN_OK) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: wifi hw mode validation failure string=%s hw variant:%d\n", __func__,
                __LINE__, output_value->buff, radio_variant);
            return false;
        }

        uint32_t temp_channel_width = sync_bandwidth_and_hw_variant(radio_variant,
            dm_radio_param->channelWidth);
        if (temp_channel_width != 0) {
            wifi_util_info_print(WIFI_DMCLI, "%s:%d:change bandwidth from %d to %d\r\n", __func__,
                __LINE__, dm_radio_param->channelWidth, temp_channel_width);
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:variant=%d  str_variant=%s\n", __func__, __LINE__,
            radio_variant, output_value->buff);
        is_radio_config_changed = true;
    } else if (STR_CMP(param_name, "OperatingChannelBandwidth")) {
        ...
        if ((tmp_chan_width == WIFI_CHANNELBANDWIDTH_160MHZ) &&
            (dm_radio_param->band == WIFI_FREQUENCY_5_BAND) && (rfc_pcfg->dfs_rfc != true)) {
            wifi_util_error_print(WIFI_DMCLI, "%s:%d: DFS Disabled!! Cannot set to chan_width=%d\n",
                __func__, __LINE__, tmp_chan_width);
            return false;
        }
        ...
        if (dm_radio_param->band == WIFI_FREQUENCY_2_4_BAND) {
            if ((tmp_chan_width != WIFI_CHANNELBANDWIDTH_20MHZ) &&
                ...
                wifi_util_error_print(WIFI_DMCLI, "%s:%d:Cannot set chan_width=%d for band:%d\n",
                    __func__, __LINE__, tmp_chan_width, dm_radio_param->band);
                return false;
            }
        }

        if (is_bandwidth_and_hw_variant_compatible(dm_radio_param->variant, tmp_chan_width) !=
            true) {
            wifi_util_error_print(WIFI_DMCLI, "%s:%d:chan_width=%d variant:%d\n", __func__,
                __LINE__, tmp_chan_width, dm_radio_param->variant);
            return false;
        }
        ...
        if (get_radio_guard_interval_int_from_string(output_value->buff, &tmp_guard_interval) !=
            RETURN_OK) {
            return false;
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:guardInterval=%d tmpChanWidth=%d\n", __func__,
            __LINE__, dm_radio_param->guardInterval, tmp_guard_interval);
        is_radio_config_changed = true;
    } else if (STR_CMP(param_name, "RegulatoryDomain")) {
        ...
        if (get_reg_domain_int_from_string(output_value->buff, &temp_country_code,
                &temp_oper_env) != RETURN_OK) {
            return false;
        ...
        if (radio_index == 1) {
            ...
            snprintf(dm_wifi_global_cfg->global_parameters.wifi_region_code,
                sizeof(dm_wifi_global_cfg->global_parameters.wifi_region_code), "%s",
                (char *)output_value->buff);
            g_update_wifi_region = true;
            if ((RETURN_OK == get_partner_id(partner_id)) && (partner_id[0] != '\0')) {
                if (update_json_param("Device.WiFi.X_RDKCENTRAL-COM_Syndication.WiFiRegion.Code",
                        partner_id, output_value->buff, requestor_str, current_time) != RETURN_OK) {
                    wifi_util_error_print(WIFI_DMCLI,
                        "%s:%d Unable to update WifiRegion to Json file\n", __func__, __LINE__);
                }
            }
        ...
        wifi_util_dbg_print(WIFI_DMCLI, "%s:%d: country code=%d  environment=%d  input=%s\n",
            __func__, __LINE__, dm_radio_param->countryCode, dm_radio_param->operatingEnvironment,
            output_value->buff);
        is_radio_config_changed = true;
    } else if (STR_CMP(param_name, "BasicDataTransmitRates")) {
        uint32_t tx_rate = 0;
        ...
        if (is_valid_transmit_rate(output_value->buff)) {
            ...
            wifi_util_dbg_print(WIFI_DMCLI, "%s:%d:operational Data Transmit Rates = %d\n",
                __func__, __LINE__, dm_radio_param->operationalDataTransmitRates);
            is_radio_config_changed = true;
        }
    } else if (STR_CMP(param_name, "Alias")) {
        dml_radio_default *dm_radio_default = get_radio_default_obj(radio_index);
...
bool accesspoint_get_param_bool_value(void *obj_ins_context, char *param_name, bool *output_value)
{
    ...
    if (STR_CMP(param_name, "Enable")) {
        ...
        *output_value = pcfg->u.bss_info.UAPSDEnabled;
        ...
        wifi_platform_property_t *p_wifi_prop =
            &((webconfig_dml_t *)get_webconfig_dml())->hal_cap.wifi_prop;
        ...
        if (isVapSTAMesh(pcfg->vap_index)) {
            ...
            *output_value = pcfg->u.bss_info.mgmtPowerControl;
            wifi_util_info_print(WIFI_DMCLI,
                "RDK_LOG_INFO,X_RDKCENTRAL-COM_ManagementFramePowerControl:%d\n",
                pcfg->u.bss_info.mgmtPowerControl);
            wifi_util_info_print(WIFI_DMCLI,
                "X_RDKCENTRAL-COM_ManagementFramePowerControl_Get:<%d>\n",
                pcfg->u.bss_info.mgmtPowerControl);
        }
...
bool accesspoint_set_param_bool_value(void *obj_ins_context, char *param_name, bool output_value)
{
    ...
    if (STR_CMP(param_name, "Enable")) {
        ...
        if (p_dm_vap_info->u.bss_info.interworking.interworking.interworkingEnabled ==
            output_value) {
            return true;
        }
        ...
        if (output_value == true) {
            wifi_util_dbg_print(WIFI_DMCLI,
                "%s:%d X_RDKCENTRAL-COM_InterworkingApplySettings push to queue \n", __func__,
                __LINE__);
            if (push_vap_dml_cache_to_one_wifidb() == RETURN_ERR) {
                wifi_util_dbg_print(WIFI_DMCLI,
                    "%s:%d X_RDKCENTRAL-COM_InterworkingApplySettings falied \n", __func__,
                    __LINE__);
                return false;
            }
            last_vap_change = get_current_time_in_sec();
        ...
        if (!isVapHotspot(instance_number - 1)) {
            wifi_util_error_print(WIFI_DMCLI,
                "RDK_LOG_ERROR, %s connected_building_enabled  not supported for vaps other than "
                "public vaps\n",
                __func__);
            return false;
        }
...
bool accesspoint_set_param_int_value(void *obj_ins_context, char *param_name, int output_value)
{
    ...
    if (STR_CMP(param_name, "X_CISCO_COM_WmmNoAck")) {
        ...
        if ((output_value < -20) || (output_value > 0)) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d Unsupported Value for ManagementFramePowerControl"
    ...
    uint32_t output_value)
{
    ...
    if (STR_CMP(param_name, "RetryLimit")) {
        ...
        if (isVapSTAMesh(p_dm_vap_info->vap_index)) {
            wifi_util_info_print(WIFI_DMCLI, "%s:%d %s does not support configuration\n", __func__,
                __LINE__, p_dm_vap_info->vap_name);
            return TRUE;
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "X_RDKCENTRAL-COM_BeaconRate")) {
        ...
        if (get_beacon_rate_int_from_string((char *)output_value->buff, &beacon_rate_type) !=
            RETURN_OK) {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "SSID")) {
        ...
        if (isVapSTAMesh(pcfg->vap_index)) {
            sprintf(buff, "%02X:%02X:%02X:%02X:%02X:%02X", pcfg->u.sta_info.bssid[0],
                pcfg->u.sta_info.bssid[1], pcfg->u.sta_info.bssid[2], pcfg->u.sta_info.bssid[3],
                pcfg->u.sta_info.bssid[4], pcfg->u.sta_info.bssid[5]);
        } else {
            sprintf(buff, "%02X:%02X:%02X:%02X:%02X:%02X", pcfg->u.bss_info.bssid[0],
                pcfg->u.bss_info.bssid[1], pcfg->u.bss_info.bssid[2], pcfg->u.bss_info.bssid[3],
        ...
        if (isVapSTAMesh(pcfg->vap_index)) {
            sprintf(buff, "%02X:%02X:%02X:%02X:%02X:%02X", pcfg->u.sta_info.mac[0],
                pcfg->u.sta_info.mac[1], pcfg->u.sta_info.mac[2], pcfg->u.sta_info.mac[3],
...
bool ssid_set_param_bool_value(void *obj_ins_context, char *param_name, bool output_value)
{
    ...
    if (STR_CMP(param_name, "Enable")) {
        ...
        dm_rdk_vap_info = (rdk_wifi_vap_info_t *)get_dml_cache_rdk_vap_info(
            p_dm_vap_info->vap_index);
        ...
        if (!(dm_wifi_global_cfg->global_parameters.force_disable_radio_feature)) {
            ...
            wifi_util_dbg_print(WIFI_DMCLI,
                "%s:%d %s WIFI_ATTEMPT_TO_CHANGE_CONFIG_WHEN_FORCE_DISABLED\n", __func__, __LINE__,
                pcfg->vap_name);
            return false;
        }
    } else if (STR_CMP(param_name, "X_CISCO_COM_EnableOnline")) {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "SSID")) {
        ...
        if (isVapSTAMesh(p_dm_vap_info->vap_index)) {
            ...
            snprintf(p_dm_vap_info->u.sta_info.ssid, sizeof(p_dm_vap_info->u.sta_info.ssid), "%s",
                (char *)output_value->buff);
            set_dml_cache_vap_config_changed(instance_number - 1);
            return true;
        ...
        snprintf(p_dm_vap_info->u.bss_info.ssid, sizeof(p_dm_vap_info->u.bss_info.ssid), "%s",
            (char *)output_value->buff);
        set_dml_cache_vap_config_changed(instance_number - 1);
    } else {
        wifi_util_info_print(WIFI_DMCLI, "%s:%d: unsupported param name:%s\n", __func__, __LINE__,
...
bool security_get_param_bool_value(void *obj_ins_context, char *param_name, bool *output_value)
{
    ...
    if (isVapSTAMesh(vap_index)) {
        ...
        if (p_sec_cfg == NULL) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: %s invalid Get_wifi_object_sta_security_parameter\n", __func__, __LINE__,
                pcfg->vap_name);
            return false;
        }
    } else {
        ...
        if (p_sec_cfg == NULL) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: %s invalid get_dml_cache_security_parameter\n", __func__, __LINE__,
                pcfg->vap_name);
            return false;
        }
    }
    wifi_radio_operationParam_t *p_dm_radio_param =
        (wifi_radio_operationParam_t *)get_dml_cache_radio_map(pcfg->radio_index);
...
bool security_set_param_bool_value(void *obj_ins_context, char *param_name, bool output_value)
{
    ...
    uint8_t vap_index = convert_vap_name_to_index(
        &((webconfig_dml_t *)get_webconfig_dml())->hal_cap.wifi_prop, pcfg->vap_name);
    wifi_radio_operationParam_t *p_dm_radio_param = get_dml_cache_radio_map(pcfg->radio_index);
    DM_CHECK_NULL_WITH_RC(p_dm_radio_param, false);
    ...
    if (isVapSTAMesh(vap_index)) {
        ...
        if (p_dm_sec_cfg == NULL) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: %s invalid get dml cache sta security parameter\n", __func__, __LINE__,
                pcfg->vap_name);
            return false;
        }
    } else {
        ...
        if (p_dm_sec_cfg == NULL) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d: %s invalid get dml cache bss security parameter \n", __func__, __LINE__,
                pcfg->vap_name);
            return false;
        }
    }
...
bool security_set_param_uint_value(void *obj_ins_context, char *param_name, uint32_t output_value)
{
    ...
    if (STR_CMP(param_name, "RekeyingInterval")) {
        ...
        if (!security_mode_support_radius(p_dm_sec_cfg->mode)) {
            wifi_util_info_print(WIFI_DMCLI,
                "%s:%d Security mode %d does not support radius configuration\n", __func__,
    ...
    if (STR_CMP(param_name, "ModeEnabled")) {
        ...
        if (p_dm_radio_param->band != WIFI_FREQUENCY_6_BAND && rfc_pcfg->wpa3_rfc == false &&
            (l_tmp_sec_mode == wifi_security_mode_wpa3_transition ||
                l_tmp_sec_mode == wifi_security_mode_wpa3_personal)) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d WPA3 mode is not supported when "
        ...
        if ((output_value->buff_len < 8) || (output_value->buff_len > 63)) {
            wifi_util_error_print(WIFI_DMCLI, "%s:%d Size is too large value=%s\n", __func__,
                __LINE__, output_value->buff, output_value->buff_len);
            return false;
        }
        ...
        if (security_mode_support_radius(p_dm_sec_cfg->mode)) {
            wifi_util_error_print(WIFI_DMCLI,
                "%s:%d Security mode %d does not support passphrase configuration \n", __func__,
                __LINE__, p_dm_sec_cfg->mode);
            return false;
        }
        ...
        if (p_dm_sec_cfg->encr != l_sec_encr_type) {
            wifi_util_dbg_print(WIFI_DMCLI, "%s:%d: sec encryption method=%d Value=%d\n", __func__,
                __LINE__, p_dm_sec_cfg->encr, l_sec_encr_type);
            /* collect value */
        ...
        if (security_mode_support_radius(p_dm_sec_cfg->mode)) {
            wifi_util_info_print(WIFI_DMCLI,
                "%s:%d Security mode %d does not support"
        ...
        if ((parameter_ip.family == wifi_ip_family_ipv4) &&
            (parameter_ip.u.IPv4addr == p_dm_sec_cfg->u.radius.dasip.u.IPv4addr)) {
            return true;
        }

        if ((parameter_ip.family == wifi_ip_family_ipv6) &&
            (!memcmp(p_dm_sec_cfg->u.radius.dasip.u.IPv6addr, parameter_ip.u.IPv6addr, 16))) {
            return true;
        }
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "MACAddress")) {
        ...
        sprintf(buff, "%02X:%02X:%02X:%02X:%02X:%02X", p_acl_entry->mac[0], p_acl_entry->mac[1],
            p_acl_entry->mac[2], p_acl_entry->mac[3], p_acl_entry->mac[4], p_acl_entry->mac[5]);
...
{
    wifi_util_info_print(WIFI_DMCLI, "%s:%d: param name:%s\n", __func__, __LINE__, param_name);
    mac_filter_set_param_arg_t *p_mac_filter_set_arg =
        (mac_filter_set_param_arg_t *)obj_ins_context;
    ...
    if (*acl_new_entry_queue == NULL) {
        wifi_util_dbg_print(WIFI_DMCLI,
            "%s:%d Unexpected ERROR!!! acl_new_entry_queue"
            " should not be NULL\n",
            __func__, __LINE__);
    ...
    if (STR_CMP(param_name, "Internet")) {
        ...
        if (p_dm_interworking_info->interworking.hessOptionPresent != output_value) {
            wifi_util_dbg_print(WIFI_DMCLI, "%s:%d hessOptionPresent=%d output_value=%d\n",
                __func__, __LINE__, p_dm_interworking_info->interworking.hessOptionPresent,
                output_value);
    ...
    if (STR_CMP(param_name, "AccessNetworkType")) {
        ...
        if (update_invalid_type == false) {
            ...
            wifi_util_info_print(WIFI_DMCLI, "%s:%d: invalid config:%d for param name:%s\n",
                __func__, __LINE__, output_value, param_name);
            return false;
        }
    } else if (STR_CMP(param_name, "VenueInfo.Group")) {
        if (output_value < 12) {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "MACAddress")) {
        char p_mac[18];
        snprintf(p_mac, 18, "%02x:%02x:%02x:%02x:%02x:%02x",
            assoc_dev_data->dev_stats.cli_MACAddress[0],
            assoc_dev_data->dev_stats.cli_MACAddress[1],
            assoc_dev_data->dev_stats.cli_MACAddress[2],
            assoc_dev_data->dev_stats.cli_MACAddress[3],
            assoc_dev_data->dev_stats.cli_MACAddress[4],
        ...
        if (p_dm_vap_info->u.bss_info.wpsPushButton != output_value) {
            ...
            if (output_value == true) {
                ...
                if ((p_dm_vap_default->wps_methods & WIFI_ONBOARDINGMETHODS_PUSHBUTTON) == 0) {
                    wifi_util_error_print(WIFI_DMCLI,
                        "(%s) WPS PBC:%d is not configured for vap:%d\n", __func__,
            ...
            if (output_value == true) {
                ...
                push_event_to_ctrl_queue(&temp_vap_index, sizeof(temp_vap_index),
                    wifi_event_type_command, wifi_event_type_command_wps, NULL);
                p_dm_vap_info->u.bss_info.wpsPushButton = false;
            }
        }
    } else if (STR_CMP(param_name, "X_CISCO_COM_CancelSession")) {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "ConfigMethodsEnabled")) {
        ...
        if (get_wifi_wps_method_int_from_string((char *)output_value->buff, &l_wps_method) !=
            RETURN_OK) {
        ...
        if ((strlen((char *)output_value->buff) >= 4) &&
            (strlen((char *)output_value->buff) <= 8)) {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "Parameters")) {
        if (STR_CMP((char *)p_dm_interworking_info->anqp.anqpParameters,
                (char *)output_value->buff)) {
            return true;
        } else {
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "Parameters")) {
        ...
        WiFi_GetWANMetrics((vap_pcfg->vap_index + 1),
            (char *)&p_interworking_info->passpoint.wanMetricsInfo,
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "Parameters")) {
        if (STR_CMP((char *)p_dm_interworking_info->passpoint.hs2Parameters,
                (char *)output_value->buff)) {
            return true;
        } else {
            cJSON *p_root = cJSON_Parse((char *)output_value->buff);
            ...
            STR_COPY((char *)p_dm_interworking_info->passpoint.hs2Parameters,
                (char *)output_value->buff);
            set_dml_cache_vap_config_changed(instance_number - 1);
            cJSON_Delete(p_root);
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if ((isVapSTAMesh(vap_pcfg->vap_index)) || (!isVapHotspot(vap_pcfg->vap_index))) {
        wifi_util_info_print(WIFI_DMCLI, "%s:%d %s does not support configuration\n", __func__,
            __LINE__, vap_pcfg->vap_name);
        return true;
    }
    ...
    if (STR_CMP(param_name, "RssiUpThreshold")) {
        ...
        if (STR_CMP(p_input_str, p_dm_pre_assoc->minimum_advertised_mcs)) {
            ...
            if (ret != 1) {
                wifi_util_info_print(WIFI_DMCLI,
                    "%s:%d Incorrect format: value should be"
            ...
            if (val < 0 || val > 7) {
                wifi_util_info_print(WIFI_DMCLI,
                    "%s:%d Incorrect value, value should be"
                    " within 0 to 7\n",
                    __func__, __LINE__);
                return false;
            }
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "Radio")) {
        ...
        if (get_radio_band_int_from_string(p_neighbor_cfg->ap_OperatingFrequencyBand,
                &l_freq_band) != RETURN_ERR) {
            return false;
        }
...
{
    wifi_util_info_print(WIFI_DMCLI, "%s:%d: param name:%s\n", __func__, __LINE__, param_name);
    instant_measurement_config_t *p_dml_harvester =
    ...
    if (STR_CMP(param_name, "ReportingPeriod")) {
        if (validate_def_reporting_period_value(output_value)) {
            if (pcfg->b_inst_client_enabled && (output_value != 0) &&
                ...
                wifi_util_error_print(WIFI_DMCLI,
                    "%s:%d Unsupported parameter value:%d"
    ...
    if (STR_CMP(param_name, "OverrideTTL")) {
        if (pcfg->b_inst_client_enabled && (output_value != 0) &&
            ...
            wifi_util_error_print(WIFI_DMCLI,
                ...
                __func__, __LINE__, output_value, p_dml_harvester->u_inst_client_reporting_period);
            return false;
    ...
    scratch_data_buff_t *output_value)
{
    ...
    if (STR_CMP(param_name, "Code")) {
        ...
        for (radio_index = 0; radio_index < get_num_radio_dml(); radio_index++) {
            p_dm_wifi_radio_param = (wifi_radio_operationParam_t *)get_dml_cache_radio_map(
                radio_index);
            if (p_dm_wifi_radio_param == NULL) {
                wifi_util_dbg_print(WIFI_DMCLI,
                    "%s:%d Unable to fetch Operating params for radio index:%d\n", __func__,
                    __LINE__, radio_index);
                continue;
            }
            ...
            if (get_reg_domain_int_from_string((char *)output_value->buff, &l_country_code,
                ...
                wifi_util_info_print(WIFI_DMCLI,
                    "%s:%d Unable to convert country code for radio_index %d\n", __func__, __LINE__,
                    radio_index);
                return false;
            }
        }

        strcpy(p_dm_global_wifi_cfg->global_parameters.wifi_region_code,
            (char *)output_value->buff);
        push_global_config_dml_cache_to_one_wifidb();
        ...
        if ((RETURN_OK == get_partner_id(partner_id)) && (partner_id[0] != '\0')) {
            get_cur_time_str(current_time, sizeof(current_time));
            if (update_json_param("Device.WiFi.X_RDKCENTRAL-COM_Syndication.WiFiRegion.Code",
                    partner_id, (char *)output_value->buff, requestor_str,
                    current_time) != RETURN_OK) {
