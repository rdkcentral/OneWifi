...
bus_error_t wifi_elem_num_of_table_row(char *event_name, uint32_t *table_row_size)
{
    ...
    } else if (!strncmp(event_name, ACCESSPOINT_OBJ_TREE_NAME, strlen(ACCESSPOINT_OBJ_TREE_NAME) + 1)) {
...
bus_error_t dml_get_set_param_value(dml_callback_table_t *p_dml_cb, uint8_t cb_table_type, void *obj_ins_context, char *param_name, raw_data_t *p_data)
{
    ...
    switch(p_data->data_type) {
        case bus_data_type_boolean:
            if ((cb_table_type == DML_GET_CB) && (p_dml_cb->get_bool_value != NULL)) {
                ...
                if (p_dml_cb->set_bool_value(obj_ins_context, param_name, p_data->raw_data.b) == false) {
        ...
        case bus_data_type_int32:
            if ((cb_table_type == DML_GET_CB) && (p_dml_cb->get_int_value != NULL)) {
                if (p_dml_cb->get_int_value(obj_ins_context, param_name, &p_data->raw_data.i32) == false) {
                ...
                if (p_dml_cb->set_int_value(obj_ins_context, param_name, p_data->raw_data.i32) == false) {
        ...
        case bus_data_type_uint32:
            if ((cb_table_type == DML_GET_CB) && (p_dml_cb->get_uint_value != NULL)) {
                if (p_dml_cb->get_uint_value(obj_ins_context, param_name, &p_data->raw_data.u32) == false) {
                ...
                if (p_dml_cb->set_uint_value(obj_ins_context, param_name, p_data->raw_data.u32) == false) {
...
bus_error_t wifi_get_param_value(char *event_name, raw_data_t *p_data,struct bus_user_data * user_data )
{
    ...
    dml_callback_table_t dml_data_cb = {
        wifi_get_param_bool_value, wifi_get_param_int_value, wifi_get_param_uint_value, wifi_get_param_string_value,
        wifi_set_param_bool_value, wifi_set_param_int_value, wifi_set_param_uint_value, wifi_set_param_string_value
    };
...
bus_error_t wifi_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)pcfg, extension, p_data);
    if (status != bus_error_success) {
...
bus_error_t neig_wifi_diag_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_global_param_t *pcfg = (wifi_global_param_t *) get_wifidb_wifi_global_param();
    dml_callback_table_t dml_data_cb = {
        neig_wifi_diag_get_param_bool_value, neig_wifi_diag_get_param_int_value,
        neig_wifi_diag_get_param_uint_value, neig_wifi_diag_get_param_string_value,
        neig_wifi_diag_set_param_bool_value, neig_wifi_diag_set_param_int_value,
        neig_wifi_diag_set_param_uint_value, neig_wifi_diag_set_param_string_value
    };
...
bus_error_t neig_wifi_diag_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t neig_diag_result_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data )
{
    ...
    uint32_t index = 0;
    dml_callback_table_t dml_data_cb = {
        NULL, neig_diag_result_get_param_int_value,
        neig_diag_result_get_param_uint_value, neig_diag_result_get_param_string_value,
        NULL, NULL, NULL, NULL
    };
...
bus_error_t neig_diag_result_table_add_row_cb(char const* tableName, char const* aliasName, uint32_t* instNum)
{
    (void)aliasName;
...
bus_error_t neig_diag_result_table_remove_row_cb(char const* rowName)
{
    ...
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s:index:%d\r\n", __func__,
        __LINE__, rowName, p_dml_param->table_neig_diag_result_index);
    return bus_error_success;
}

bus_error_t neig_diag_result_event_sub_cb(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t radio_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_dml_data_model_t *p_dml_param = get_dml_data_model_param();
    dml_callback_table_t dml_data_cb = {
        radio_get_param_bool_value, radio_get_param_int_value, radio_get_param_uint_value, radio_get_param_string_value,
        radio_set_param_bool_value, radio_set_param_int_value, radio_set_param_uint_value, radio_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.Radio.%d.%s", &index, extension);
    if (p_dml_param->table_radio_index < index) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d wrong radio get index for:[%s]:%d\r\n", __func__, __LINE__, event_name, index);
        return bus_error_invalid_input;
    }
    radio_param = (wifi_radio_operationParam_t *)get_wifidb_radio_map(index - 1);

    wifi_util_info_print(WIFI_DMCLI,"%s:%d radio enable:%d event:[%s][%s]\n", __func__, __LINE__, radio_param->enable, event_name, extension);

    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)radio_param, extension, p_data);
    if (status != bus_error_success) {
...
bus_error_t radio_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)radio_param, extension, p_data);
    if (status != bus_error_success) {
...
bus_error_t radio_table_add_row_handler(char const* tableName, char const* aliasName, uint32_t* instNum)
{
    (void)aliasName;
    wifi_dml_data_model_t *p_dml_param = get_dml_data_model_param();
...
bus_error_t radio_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t accesspoint_get_param_value(char *event_name, raw_data_t *p_data,struct bus_user_data * user_data)
{
    ...
    dml_callback_table_t dml_data_cb = {
        accesspoint_get_param_bool_value, accesspoint_get_param_int_value, accesspoint_get_param_uint_value, accesspoint_get_param_string_value,
        accesspoint_set_param_bool_value, accesspoint_set_param_int_value, accesspoint_set_param_uint_value, accesspoint_set_param_string_value
    };
...
bus_error_t accesspoint_table_add_row_handler(char const* tableName, char const* aliasName, uint32_t* instNum)
{
    (void)instNum;
    (void)aliasName;
    wifi_dml_data_model_t *p_dml_param = get_dml_data_model_param();
...
bus_error_t accesspoint_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t security_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data )
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        security_get_param_bool_value, security_get_param_int_value, security_get_param_uint_value, security_get_param_string_value,
        security_set_param_bool_value, security_set_param_int_value, security_set_param_uint_value, security_set_param_string_value
    };
...
bus_error_t security_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t ssid_table_add_row_handler(char const* tableName, char const* aliasName, uint32_t* instNum)
{
    (void)instNum;
    (void)aliasName;
    wifi_dml_data_model_t *p_dml_param = get_dml_data_model_param();
...
bus_error_t ssid_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t macfilter_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    dml_callback_table_t dml_data_cb = {
        macfilter_get_param_bool_value, macfilter_get_param_int_value, macfilter_get_param_uint_value, macfilter_get_param_string_value,
        macfilter_set_param_bool_value, macfilter_set_param_int_value, macfilter_set_param_uint_value, macfilter_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.X_CISCO_COM_MacFilterTable.%d.%s", &index, &acl_index, extension);
    ...
    if (acl_entry == NULL) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d macfilter entry not found index::%d\r\n", __func__, __LINE__, acl_index);
        return status;
    }

    wifi_util_info_print(WIFI_DMCLI,"%s:%d Event:[%s][%s]\n", __func__, __LINE__, event_name, extension);
    status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)acl_entry, extension, p_data);
    if (status != bus_error_success) {
...
bus_error_t macfilter_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data )
{
    ...
    wifi_util_info_print(WIFI_DMCLI,"%s:%d Event:[%s][%s]\n", __func__, __LINE__, event_name, extension);
    status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)&l_mac_filter_args, extension, p_data);
    if (status != bus_error_success) {
...
bus_error_t macfilter_table_add_row_handler(char const* tableName, char const* aliasName, uint32_t* instNum)
{
    (void)aliasName;
    wifi_vap_info_t *vap_param;
    int vap_index = 0;
    ...
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d Added table:%s vap:%d macfilter table index:%d-%d\r\n", __func__,
        __LINE__, tableName, vap_index, *instNum, p_dml_param->table_macfilter_index[vap_param->vap_index]);
...
bus_error_t macfilter_table_remove_row_handler(char const* rowName)
{
    ...
    sscanf(rowName, "Device.WiFi.AccessPoint.%d.X_CISCO_COM_MacFilterTable.%d", &vap_index, &macfilter_entry_index);
    ...
    if (acl_entry == NULL) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d macfilter entry is not found index:%d\r\n", __func__,
            __LINE__, macfilter_entry_index);
        return ret;
    }

    int status = macfilter_tab_del_entry(vap_param, acl_entry);
    if (status != RETURN_OK) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d macfilter acl entry del failure for vap:%d index:%d\r\n", __func__,
            __LINE__, vap_index, macfilter_entry_index);
        return ret;
    }
    ...
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d rowName:%s removed for vap_index:%d index:%d\r\n", __func__,
        __LINE__, rowName, vap_index, p_dml_param->table_macfilter_index[vap_param->vap_index]);
    return bus_error_success;
}

bus_error_t macfilter_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t associated_sta_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    dml_callback_table_t dml_data_cb = {
        associated_sta_get_param_bool_value, associated_sta_get_param_int_value, associated_sta_get_param_uint_value, associated_sta_get_param_string_value,
        NULL, NULL, NULL, NULL
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.AssociatedDevice.%d.%s", &index, &sta_index, extension);

    p_assoc_sta_entry = get_sta_assoc_data_map(index, sta_index);
    wifi_util_info_print(WIFI_DMCLI,"%s:%d Event:[%s][%s]\n", __func__, __LINE__, event_name, extension);
    status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)p_assoc_sta_entry, extension, p_data);
    if (status != bus_error_success) {
...
bus_error_t associated_sta_table_add_row_handler(char const* tableName, char const* aliasName, uint32_t* instNum)
{
    (void)instNum;
    (void)aliasName;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter\r\n", __func__, __LINE__);
...
bus_error_t associated_sta_table_remove_row_handler(char const* rowName)
{
    ...
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s vap_index:%d sta_cnt:%d sta_index:%d\r\n", __func__,
        __LINE__, rowName, vap_param->vap_index, p_dml_param->table_sta_assoc_index[vap_param->vap_index], sta_index);
    return bus_error_success;
}

bus_error_t associated_sta_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t interworking_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        interworking_get_param_bool_value, interworking_get_param_int_value,
        interworking_get_param_uint_value, interworking_get_param_string_value,
        interworking_set_param_bool_value, interworking_set_param_int_value,
        interworking_set_param_uint_value, interworking_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.X_RDKCENTRAL-COM_InterworkingElement.%s", &index, extension);
...
bus_error_t interworking_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__,
        __LINE__, eventName, action, interval);
    return bus_error_success;
}
...
bus_error_t wps_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        wps_get_param_bool_value, wps_get_param_int_value, wps_get_param_uint_value, wps_get_param_string_value,
        wps_set_param_bool_value, wps_set_param_int_value, wps_set_param_uint_value, wps_set_param_string_value
    };
...
bus_error_t wps_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t ap_macfilter_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        ap_macfilter_get_param_bool_value, NULL, NULL, NULL,
...
bus_error_t ap_macfilter_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t radius_sec_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        radius_sec_get_param_bool_value, radius_sec_get_param_int_value, NULL, NULL,
        radius_sec_set_param_bool_value, radius_sec_set_param_int_value, NULL, NULL
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.Security.X_COMCAST-COM_RadiusSettings.%s", &index, extension);
...
bus_error_t radius_sec_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t auth_sec_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, auth_sec_get_param_uint_value, NULL,
        NULL, NULL, auth_sec_set_param_uint_value, NULL
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.Security.X_RDKCENTRAL-COM_Authenticator.%s", &index, extension);
...
bus_error_t auth_sec_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t conn_ctrl_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, NULL, conn_ctrl_get_param_string_value,
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d connection control param get failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t conn_ctrl_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d connection control param set failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t conn_ctrl_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t pre_conn_ctrl_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, NULL, pre_conn_ctrl_get_param_string_value,
        NULL, NULL, NULL, pre_conn_ctrl_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.ConnectionControl.PreAssocDeny.%s", &index, extension);
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d pre connection control param get failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t pre_conn_ctrl_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d pre connection control param set failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t pre_conn_ctrl_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t post_conn_ctrl_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, NULL, post_conn_ctrl_get_param_string_value,
        NULL, NULL, NULL, post_conn_ctrl_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.ConnectionControl.PostAssocDisc.%s", &index, extension);
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d post connection control param get failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t post_conn_ctrl_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d post connection control param set failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t post_conn_ctrl_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t interworking_serv_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, NULL, interworking_serv_get_param_string_value,
        NULL, NULL, NULL, interworking_serv_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.X_RDKCENTRAL-COM_InterworkingService.%s", &index, extension);
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d interwoking serv param get failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t interworking_serv_set_param_value(char *event_name, raw_data_t *p_data,struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)vap_param, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d interwoking serv param set failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t interworking_serv_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t passpoint_get_param_value(char *event_name, raw_data_t *p_data,struct bus_user_data * user_data)
{
    ...
    wifi_vap_info_t *vap_param;
    dml_callback_table_t dml_data_cb = {
        passpoint_get_param_bool_value, NULL, NULL, passpoint_get_param_string_value,
        passpoint_set_param_bool_value, NULL, NULL, passpoint_set_param_string_value
    };

    sscanf(event_name, "Device.WiFi.AccessPoint.%d.X_RDKCENTRAL-COM_Passpoint.%s", &index, extension);
...
bus_error_t passpoint_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t wifi_client_report_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    dml_callback_table_t dml_data_cb = {
        wifi_client_report_get_param_bool_value, NULL,
        wifi_client_report_get_param_uint_value, wifi_client_report_get_param_string_value,
        wifi_client_report_set_param_bool_value, NULL,
        wifi_client_report_set_param_uint_value, wifi_client_report_set_param_string_value
    };
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)pcfg, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d wifi client report param get failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t wifi_client_report_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)pcfg, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d wifi client report param set failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t wifi_client_report_event_sub_handler(char *eventName, bus_event_sub_action_t action, int32_t interval, bool* autoPublish)
{
    (void)autoPublish;
    wifi_util_dbg_print(WIFI_DMCLI,"%s:%d enter:%s: action:%d interval:%d\r\n", __func__, __LINE__, eventName, action, interval);
    return bus_error_success;
...
bus_error_t wifi_client_def_report_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data)
{
    ...
    instant_measurement_config_t *pcfg = (instant_measurement_config_t *) get_dml_harvester();
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, wifi_client_def_report_get_param_uint_value, NULL,
        NULL, NULL, wifi_client_def_report_set_param_uint_value, NULL
    };
...
bus_error_t wifi_region_code_get_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data )
{
    ...
    dml_callback_table_t dml_data_cb = {
        NULL, NULL, NULL, wifi_region_code_get_param_string_value,
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_GET_CB, (void *)pcfg, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d wifi region code param get failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
...
bus_error_t wifi_region_code_set_param_value(char *event_name, raw_data_t *p_data, struct bus_user_data * user_data )
{
    ...
    bus_error_t status = dml_get_set_param_value(&dml_data_cb, DML_SET_CB, (void *)pcfg, extension, p_data);
    if (status != bus_error_success) {
        wifi_util_error_print(WIFI_DMCLI,"%s:%d wifi region code param set failed for:[%s][%s]\r\n", __func__,
            __LINE__, event_name, extension);
    }
