...
#define OVSH_SOCKET_ONEWIFIDB "/var/tmp/wifidb.sock" //onewifi unix socket path
...
static char     ovsh_opt_db[512] = "Wifi_Rdk_Database"; //onewifi database name
static char ovsh_opt_db_path[OVSH_SOCK_PATH] = OVSH_SOCKET_ONEWIFIDB;
...
int ovsdb_connect(void)
{
    ...
    if (strlen(ovsh_opt_db_path) >= sizeof(addr.sun_path))
    {
        DEBUG("Connect path too long!\n");
        goto error;
    }
    STRSCPY(addr.sun_path, ovsh_opt_db_path);
