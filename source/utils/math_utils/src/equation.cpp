...
vector_t equation_t::arguments()
{
    ...
    for (i = 0; i < v_s_tmp.num/2; i++) {
        snprintf(v_s_signed.val[i], sizeof(expr_t), "%s%s", v_s_tmp.val[2*i], v_s_tmp.val[2*i + 1]);
        v_s_signed.num++;
    }
...
equation_t equation_t::determinant(matrix_s_t *in)
{
    ...
    if (in->rows == 2) {
        ...
        return equation_t(in->val[0][0]) * equation_t(in->val[1][1]) - equation_t(in->val[0][1]) * equation_t(in->val[1][0]);
    }
...
equation_t equation_t::operator *(equation_t e)
{
    ...
    if ((v1.m_num > 1) && (v2.m_num > 1)) {
        ...
        for (i = 0; i < v2.m_num; i++) {
            v3.m_val[i].m_re = v1.m_val[0].m_re * v2.m_val[i].m_re - v1.m_val[0].m_im * v2.m_val[i].m_im;
            v3.m_val[i].m_im = v1.m_val[0].m_im * v2.m_val[i].m_re + v1.m_val[0].m_re * v2.m_val[i].m_im;
            v3.m_num++;
        }
    } else if ((v1.m_num > 1) && (v2.m_num == 1)) {
        for (i = 0; i < v1.m_num; i++) {
            v3.m_val[i].m_re = v2.m_val[0].m_re * v1.m_val[i].m_re - v2.m_val[0].m_im * v1.m_val[i].m_im;
            v3.m_val[i].m_im = v2.m_val[0].m_im * v1.m_val[i].m_re + v2.m_val[0].m_re * v1.m_val[i].m_im;
            v3.m_num++;
        }
    } else {
        v3.m_val[0].m_re = v2.m_val[0].m_re * v1.m_val[0].m_re - v2.m_val[0].m_im * v1.m_val[0].m_im;
        v3.m_val[0].m_im = v2.m_val[0].m_re * v1.m_val[0].m_im + v2.m_val[0].m_im * v1.m_val[0].m_re;
        v3.m_num = 1;
    }
