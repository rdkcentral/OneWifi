...
webconfig_error_t encode_radio_curr_operating_classes(const wifi_radio_operationParam_t *oper, cJSON *radio_object)
{
    cJSON *obj_array, *obj;
...
webconfig_error_t encode_radio_object(const rdk_wifi_radio_t *radio, cJSON *radio_object)
{
    ...
    if (encode_radio_curr_operating_classes(radio_info, radio_object) != webconfig_error_none) {
        wifi_util_error_print(WIFI_WEBCONFIG,"%s:%d Radio current operation class encoding failed\n", __func__, __LINE__);
        return webconfig_error_encode;
    }
    return webconfig_error_none;
}
