...
static webconfig_error_t decode_anqp_object_impl(const cJSON *anqp, wifi_interworking_t *interworking_info, cJSON *passPointStats, cJSON *statsList)
{
    ...
    if (!cJSON_PrintPreallocated(passPointStats, (char *)&interworking_info->anqp.passpointStats, sizeof(interworking_info->anqp.passpointStats), false)) {
        wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: Failed to print passPointStats (truncated or OOM)\n", __func__, __LINE__);
        return webconfig_error_decode;
    }
    return webconfig_error_none;
}
...
webconfig_error_t decode_anqp_object(const cJSON *anqp, wifi_interworking_t *ii)
{
    ...
    cJSON *mainEntry = cJSON_AddObjectToObject(passPointStats, "PassPointStats");
    if (!mainEntry) {
        wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: Failed to add PassPointStats object (OOM)\n", __func__, __LINE__);
        cJSON_Delete(passPointStats);
        return webconfig_error_decode;
    }

    cJSON *statsList = cJSON_AddArrayToObject(mainEntry, "ANQPResponse");
    if (!statsList) {
        wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: Failed to add ANQPResponse array (OOM)\n", __func__, __LINE__);
        cJSON_Delete(passPointStats);
        return webconfig_error_decode;
    }

    webconfig_error_t rc = decode_anqp_object_impl(anqp, ii, passPointStats, statsList);
    cJSON_Delete(passPointStats);
    return rc;
}
...
static webconfig_error_t decode_radio_chan_entry(cJSON *radio_stats, radio_chan_data_t *out)
{
    const cJSON *param;
...
webconfig_error_t decode_radio_channel_radio_stats_object(wifi_provider_response_t **chan_stats, cJSON *json)
{
    ...
    cJSON *radio_stats;
    const cJSON  *param_radio;
    const cJSON  *param_scan;
    int size = 0;
    ...
    *chan_stats = NULL;
    ...
    decode_param_integer(json, "RadioIndex", param_radio);

    decode_param_string(json, "ScanMode", param_scan);
    if (scan_mode_type_conversion(&scan_mode, param_scan->valuestring, MAX_SCAN_MODE_LEN, STRING_TO_ENUM) != RETURN_OK)  {
        wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: scan_mode_type_conversion failed for %s\n", __func__, __LINE__, param_scan->valuestring);
        return webconfig_error_decode;
    }
    ...
    *chan_stats = (wifi_provider_response_t*) calloc(1, sizeof(wifi_provider_response_t));
    ...
    (*chan_stats)->args.radio_index = param_radio->valuedouble;
    (*chan_stats)->args.scan_mode = scan_mode;

    if (size == 0) {
        (*chan_stats)->stat_pointer = NULL;
        (*chan_stats)->stat_array_size = 0;
        wifi_util_info_print(WIFI_WEBCONFIG, "%s:%d: Radio stats array size is %d\n", __func__, __LINE__, (*chan_stats)->stat_array_size);
        return webconfig_error_none;
    }

    chan_data = (radio_chan_data_t*) calloc(size, sizeof(radio_chan_data_t));
    if (chan_data == NULL) {
        free(*chan_stats);
        *chan_stats = NULL;
        wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d Failed to allocate memory\n", __func__, __LINE__);
    ...
    for (int count = 0; count < size; count++) {
        ...
        if (decode_radio_chan_entry(radio_stats, &chan_data[count]) != webconfig_error_none) {
            free(chan_data);
            free(*chan_stats);
            *chan_stats = NULL;
            return webconfig_error_decode;
...
static webconfig_error_t decode_neighbor_stats_entry(cJSON *neighbor_stats, wifi_neighbor_ap2_t *out)
{
    const cJSON *param;
    if ((neighbor_stats == NULL) || (out == NULL)) {
    ...
    if ((param != NULL) && cJSON_IsString(param) && (param->valuestring != NULL)) {
        strncpy(out->ap_SSID, param->valuestring, sizeof(out->ap_SSID) - 1);
    }

    decode_param_string(neighbor_stats, "ap_BSSID", param);
    strncpy(out->ap_BSSID, param->valuestring, sizeof(out->ap_BSSID) - 1);

    decode_param_string(neighbor_stats, "ap_Mode", param);
    strncpy(out->ap_Mode, param->valuestring, sizeof(out->ap_Mode) - 1);

    decode_param_integer(neighbor_stats, "ap_Channel", param);
    out->ap_Channel = param->valuedouble;

    decode_param_integer(neighbor_stats, "ap_SignalStrength", param);
    out->ap_SignalStrength = param->valuedouble;

    decode_param_string(neighbor_stats, "ap_SecurityModeEnabled", param);
    strncpy(out->ap_SecurityModeEnabled, param->valuestring, sizeof(out->ap_SecurityModeEnabled) - 1);

    decode_param_string(neighbor_stats, "ap_EncryptionMode", param);
    strncpy(out->ap_EncryptionMode, param->valuestring, sizeof(out->ap_EncryptionMode) - 1);

    decode_param_string(neighbor_stats, "ap_OperatingFrequencyBand", param);
    strncpy(out->ap_OperatingFrequencyBand, param->valuestring, sizeof(out->ap_OperatingFrequencyBand) - 1);

    decode_param_string(neighbor_stats, "ap_SupportedStandards", param);
    strncpy(out->ap_SupportedStandards, param->valuestring, sizeof(out->ap_SupportedStandards) - 1);

    decode_param_string(neighbor_stats, "ap_OperatingStandards", param);
    strncpy(out->ap_OperatingStandards, param->valuestring, sizeof(out->ap_OperatingStandards) - 1);

    decode_param_string(neighbor_stats, "ap_OperatingChannelBandwidth", param);
    strncpy(out->ap_OperatingChannelBandwidth, param->valuestring, sizeof(out->ap_OperatingChannelBandwidth) - 1);

    decode_param_integer(neighbor_stats, "ap_BeaconPeriod", param);
    out->ap_BeaconPeriod = param->valuedouble;

    decode_param_integer(neighbor_stats, "ap_Noise", param);
    out->ap_Noise = param->valuedouble;

    decode_param_string(neighbor_stats, "ap_BasicDataTransferRates", param);
    strncpy(out->ap_BasicDataTransferRates, param->valuestring, sizeof(out->ap_BasicDataTransferRates) - 1);

    decode_param_string(neighbor_stats, "ap_SupportedDataTransferRates", param);
    strncpy(out->ap_SupportedDataTransferRates, param->valuestring, sizeof(out->ap_SupportedDataTransferRates) - 1);

    decode_param_integer(neighbor_stats, "ap_DTIMPeriod", param);
    out->ap_DTIMPeriod = param->valuedouble;

    decode_param_integer(neighbor_stats, "ap_ChannelUtilization", param);
    out->ap_ChannelUtilization = param->valuedouble;
...
webconfig_error_t decode_radio_neighbor_stats_object(wifi_provider_response_t **chan_stats, cJSON *json)
{
    ...
    *chan_stats = (wifi_provider_response_t*) calloc(1, sizeof(wifi_provider_response_t));
    ...
    for (int count = 0; count < size; count++) {
        ...
        if (decode_neighbor_stats_entry(neighbor_stats, &neighbor_stats_data[count]) != webconfig_error_none) {
            free(neighbor_stats_data);
            free(*chan_stats);
            *chan_stats = NULL;
            return webconfig_error_decode;
