...
telemetry_data_t *create_wpa3_sta_data_hash_map(hash_map_t *sta_map, mac_addr_t l_sta_mac) {
    ...
    wifi_util_info_print(WIFI_MON, "Created STA entry for MAC: %s\r\n", mac_str_dup);
    pthread_mutex_unlock(&g_monitor_module.data_lock);
    return sta;
}

hash_map_t *get_wpa3_sta_data_map(unsigned int vap_index) {
    ...
    pthread_mutex_unlock(&g_monitor_module.data_lock);
    wifi_util_info_print(WIFI_MON, "Retrieved STA data map for VAP index: %d\r\n", vap_index);
    return g_monitor_module.bssid_data[vap_array_index].wpa3_sta_map;
}

int set_wpa3_assoc_frame_data(frame_data_t *msg) {
...
void telemetry_event_wpa3(int vapindex, char *mac, int rsnvariant, frame_type_t frame_type, int key_mgmt) {
    ...
    snprintf(telemetry_val, sizeof(telemetry_val),
             "%d,%s,%d,%d,%d", vapindex, mac, rsnvariant, frame_type, key_mgmt);
    strncpy(telemetry_buff_grep, telemetry_buff, sizeof(telemetry_buff_grep) - 1);
    telemetry_buff_grep[sizeof(telemetry_buff_grep) - 1] = '\0';
    wifi_util_dbg_print(WIFI_MON, "%s:%s\n", telemetry_buff_grep, telemetry_val);
    get_stubs_descriptor()->t2_event_s_fn(telemetry_buff, telemetry_val);
...
int set_sta_client_mode(int ap_index, char *mac, int key_mgmt, frame_type_t frame_type, int band) {
    ...
    if (frame_type == ASSOC_REQUEST || frame_type == REASSOC_REQUEST) {
	    ...
	    report_connection_event(sta1, mode, sta->eapol_akm);
            telemetry_event_akm_count(sta1, ap_index, mac);
        }
        else {
    ...
    assoc_dev_data_t *assoc_data)
{
    ...
    if (sta != NULL) {
        if (sta->assoc_frame_data.msg_data.frame.len != 0) {
            ...
            wifi_util_dbg_print(WIFI_MON,"%s:%d sta found for mac:%s harsha stationmode:%d eapomode:%d \n", __func__, __LINE__, mac_addr,sta->assoc_akm,sta->eapol_akm);
