...
wifi_mon_collector_element_t * coordinator_create_collector_elem(wifi_mon_stats_config_t *stats_config, wifi_mon_stats_descriptor_t *stat_desc)
{
    ...
    wifi_util_error_print(WIFI_MON,"SJY %s:%d The value of survey_interval_ms before memcpy is %d", stats_config->args.dwell_time);
    memcpy(collector_elem->args, &stats_config->args, sizeof(wifi_mon_stats_args_t));
    wifi_util_error_print(WIFI_MON,"SJY %s:%d The value of survey_interval_ms after memcpy is %d", collector_elem->args->dwell_time);
