...
{  
    ...
    memcpy(&mon_data->bssid_data[args->vap_index].vap_traffic, vap_stats, sizeof(vap_traffic_stats_t));  
                              ...
                              unsigned int *stat_array_size, wifi_monitor_t *mon_cache)  
{
    ...
    memcpy(out, &mon_cache->bssid_data[p_elem->mon_stats_config->args.vap_index].vap_traffic, sizeof(vap_traffic_stats_t));  
    pthread_mutex_unlock(&mon_cache->data_lock);  
