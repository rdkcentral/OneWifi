...
int web_t::send_message(int fd, web_data_extn_type_t data_type, bool file, const char *data)
{
    ...
    if((fdfile = open(data, O_RDONLY)) < 0){
        printf("%s:%d: Cannot Open file path :%s with error %d\n", __func__, __LINE__, data, fdfile);
...
int web_t::server(int sock)
{
    ...
    printf("%s:%d:waiting for child data\n", __func__, __LINE__);
    valread = read(sock, buffer, sizeof(buffer));
    if (valread <= 0) {
        printf("%s:%d client stop:%d\r\n", __func__, __LINE__, valread);
        close(sock);
        return 0;
    }
    ...
    //printf("%s:%d: Method: %s\tString: %s\n", __func__, __LINE__, method_string, action_string);
...
int web_t::run()
{
    ...
    while (m_exit == false) {
        ...
        if (pthread_create(&stid, &attr, server, args) != 0) {
            printf("%s:%d: Failed to start child thread\n", __func__, __LINE__);
            return -1;
        } else {
            pthread_detach(stid);
            printf("%s:%d:started child thread\n", __func__, __LINE__);
        }
    }
...
int web_t::start()
{
    ...
    printf("%s:%d web server start\r\n", __func__, __LINE__);
    if (init() != 0) {
        printf("%s:%d: Error in initializing server\n", __func__, __LINE__);
        return -1;
    }
    
    // Creating socket file descriptor
    if ((m_server_fd = socket(AF_UNIX, SOCK_STREAM, 0)) == 0) {
        printf("%s:%d: Error in cfreating socket: %d\n", __func__, __LINE__, errno);
        return -1;
    } else {
        printf("%s:%d web server socket created:%d\r\n", __func__, __LINE__, m_server_fd);
    }

    if (bind(m_server_fd, (struct sockaddr *)&m_address, sizeof(struct sockaddr_un)) < 0) {
        printf("%s:%d: Error in binding: %d\n", __func__, __LINE__, errno);
        close(m_server_fd);
        return -1;
    }
    ...
    if (pthread_create(&tid, &attr, run, this) != 0) {
        ...
        pthread_detach(tid);
        printf("%s:%d web server run pthread create\r\n", __func__, __LINE__);
    }
