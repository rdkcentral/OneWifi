...
vector_t matrix_t::faddeev_leverrier()
{
    ...
    for (i = 0; i < m_rows; i++) {
        n = number_t(pow(i + 1, -1), 0);
        p.m_val[i + 1] = n * m.trace();
        m = (*this)*(m - p.m_val[i + 1]*u);
    }
    
    for (i = 1; i < p.m_num; i++) {
        p.m_val[i] = number_t(0, 0) - p.m_val[i];
    }
...
matrix_t matrix_t::operator *(const matrix_t& m) const
{
    ...
    for (i = 0; i < out.m_rows; i++) {
        for (j = 0; j < out.m_cols; j++) {
            for (k = 0; k < m_cols; k++) {
                out.m_val[i][j].m_re += (m_val[i][k].m_re * m.m_val[k][j].m_re - m_val[i][k].m_im * m.m_val[k][j].m_im);
                out.m_val[i][j].m_im += (m_val[i][k].m_re * m.m_val[k][j].m_im + m_val[i][k].m_im * m.m_val[k][j].m_re);
            }
        }
    }
