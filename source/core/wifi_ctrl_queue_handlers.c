...
void update_lm_wifi_sync_host_AssociatedDevice_DM_refs(unsigned int vap_index, LM_wifi_host_t *host, assoc_dev_data_t *assoc_data)
{
        ...
        for (int link_idx = 0; link_idx < MAX_NUM_RADIOS; link_idx++) {
            if (assoc_data->mld_info.cli_LinkInfo[link_idx].cli_Valid) {
                ...
                if (strlen(str) + strlen(assoc_device_str) + 2 < sizeof(str)) { /*+1 for ';' +1 for '\0'*/
                    ...
                    wifi_util_error_print(WIFI_CTRL,
                        "%s:%d AssociatedDevice string truncated for MAC %s\n", __func__, __LINE__,
                        assoc_data->dev_stats.cli_MACAddress);
                    break;
                }
            }
        }
    ...
    //Stano: this is redundant, we can directly write to dest buffer?
...
int process_device_removal(rdk_wifi_vap_info_t *rdk_vap_info,
                            ...
                            ULONG old_count)
{
    wifi_util_dbg_print(WIFI_CTRL,"Stano Removing MAC %s from vap_index %d assoc link %d\n",
            mac_str, rdk_vap_info->vap_index, removed_dev->association_link);
...
static void get_AssociatedDevice_DM_ref(int vap_index, assoc_dev_data_t *assoc_data, char* str, size_t str_size)
{
    ...
    if (rdk_vap_info->associated_devices_map != NULL) {
        ...
        } else { //Stano???: when the dev is not found - hash_map_count + 1 ???
            itrj = hash_map_count(rdk_vap_info->associated_devices_map);
...
static void get_AssociatedDevice_DM_MLO_refs(assoc_dev_data_t *assoc_data, char *str, size_t str_size)
{
    for (int link_idx = 0; link_idx < MAX_NUM_RADIOS; link_idx++) {
        if (assoc_data->mld_info.cli_LinkInfo[link_idx].cli_Valid) {
            ...
            if (strlen(str) + strlen(assoc_device_str) + 2 < str_size) { /*+1 for ';' +1 for '\0'*/
                if (strlen(str) > 0) {
                    strcat(str, ";");
                }
                strcat(str, assoc_device_str);
...
void process_assoc_device_event(void *data)
{
     ...
     * so we need to create/send the LM lite notification when all links are added to VAPs assoc_dev maps*/
