...
int get_managed_guest_bridge(char *brval, unsigned long length, int radio_index)
{
    ...
    wifi_util_info_print(WIFI_SRI, "Managed_wifi bridge DMLCI name is %s\n", str);
    ...
    if (rc == bus_error_success) {
        wifi_util_info_print(WIFI_SRI, "%s:%d Managed_wifi bridge DMCLI name is %s and inside rc == bus_error_success\n",__func__,__LINE__, str);
        brname = (char *)data.raw_data.bytes;
        wifi_util_info_print(WIFI_SRI, "%s:%d Managed_wifi bridge name is %s\n",__func__,__LINE__, brname);
        token = strrchr(brname, ':');
        if (token) {
            snprintf(brval, length, "%s", token + 1);
            wifi_util_info_print(WIFI_SRI,"%s:%d strrchr did not return NULL for radio index %d\n",__func__,__LINE__,radio_index);
        } else {
            snprintf(brval, length, "%s", brname); 
            wifi_util_info_print(WIFI_SRI,"%s:%d strrchr returned NULL for radio_index = %d\n",__func__,__LINE__,radio_index);
        }
        wifi_util_info_print(WIFI_SRI, "%s:%d Managed_wifi bridge val is %s\n",__func__,__LINE__,brval);
        get_bus_descriptor()->bus_data_free_fn(&data);
        return RETURN_OK;
    }
    /* Just in case if 'rc' value is not handled  correctly */
    wifi_util_info_print(WIFI_SRI, "%s:%d Managed_wifi bridge name is %s and value of rc = %d\n",__func__,__LINE__, str, rc);
    get_bus_descriptor()->bus_data_free_fn(&data);
    return RETURN_ERR;
}
