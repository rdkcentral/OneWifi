...
int webconfig_em_subdoc_config_apply(wifi_ctrl_t *ctrl, webconfig_subdoc_decoded_data_t *data)
{
    ...
    for(UINT index = 0; index < getTotalNumberVAPs(); index++)
    {
        ...
        wifi_util_error_print(WIFI_CTRL,"%s:%d: START : for loop: apIdx =%d :index =%d : radio_index =%d  \n",__func__, __LINE__,apIdx,index,vapInfo->radio_index);
        ...
        {
            if((strncmp(vapInfo->u.bss_info.ssid,data->em_config.traffic_separation_policy.ssids[0].ssid,sizeof(data->em_config.traffic_separation_policy.ssids[0].ssid))==0) && (vapInfo->vlan_id == data->em_config.traffic_separation_policy.ssids[1].vlan_id))
            {
                continue;
            }
            ...
            strncpy(vapInfo->bridge_name,"brlan1",sizeof(vapInfo->bridge_name));
            vapInfo->vlan_id = data->em_config.traffic_separation_policy.ssids[1].vlan_id;
        }
        else if(isVapXhs(apIdx))
        {
            if((strncmp(vapInfo->u.bss_info.ssid,data->em_config.traffic_separation_policy.ssids[2].ssid,sizeof(data->em_config.traffic_separation_policy.ssids[0].ssid))==0) && (vapInfo->vlan_id == data->em_config.traffic_separation_policy.ssids[2].vlan_id))
            {
                continue;
            }
            ...
            strncpy(vapInfo->bridge_name,"brlan2",sizeof(vapInfo->bridge_name));
        ...
        wifi_util_info_print(WIFI_CTRL,"%s:%d: ssid =%s vlan_id = %d:  \n",__func__, __LINE__,vapInfo->u.bss_info.ssid,vapInfo->vlan_id);
        if (rdk_vap_info == NULL) {
            wifi_util_error_print(WIFI_CTRL, "%s:%d Failed to get rdk vap info for index %d\n",__func__, __LINE__, apIdx);
            continue;
        }
