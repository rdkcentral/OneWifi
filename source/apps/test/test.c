...
static void em_topo_stream_send_topology(const char *topology_json)
{
    ...
    /* ---- Send ---- */
    {
        ...
        if (n == 0) {
            wifi_util_dbg_print(WIFI_APPS, "[TOPO-WS] DataFrame sent successfully ordering_id=%s len=%zu", id_buf, jlen);
        } else {
            wifi_util_dbg_print(WIFI_APPS, "[TOPO-WS] Send failed ordering_id=%s — closing connection", id_buf);
            em_topo_close();
        }
    }
