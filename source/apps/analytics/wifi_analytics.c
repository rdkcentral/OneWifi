        ...
        case webconfig_subdoc_type_mesh_backhaul_sta:
            ...
            for (i = 0; i < getNumberRadios(); i++) {
                ...
                for (j = 0; j < radio->vaps.num_vaps; j++) {
                    ...
                    if(vap->vap_name[0] != '\0') {
                        out_bytes += snprintf(&temp_str[out_bytes], (sizeof(temp_str)-out_bytes),"\\n%d,%s,%s,%s",
                                vap->vap_index, (vap->u.sta_info.ignite_enabled==TRUE)?vap->u.sta_info.repurposed_ssid:vap->u.sta_info.ssid, ((vap->u.sta_info.enabled==TRUE) || (vap->u.sta_info.ignite_enabled==TRUE))?"enb":"dis",
