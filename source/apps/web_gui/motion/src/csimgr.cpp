...
void csimgr_t::dump_json(cJSON *obj)
{
    ...
    wifi_util_info_print(WIFI_WEB_GUI,"%s\n", buff);
    free(buff);
}
...
void csimgr_t::update_graph(sounder_t *sd)
{
    ...
    //wifi_util_dbg_print(WIFI_WEB_GUI,"%s:%s:%d: Updating graph for device: %s, test duration: %d\n", get_local_time(scratch, sizeof(scratch)), __func__, __LINE__, str, m_args.remaining/1000);
    
    
    if ((sounder_arr_obj = cJSON_GetObjectItem(m_out_obj, "Devices")) == NULL) {
        wifi_util_error_print(WIFI_WEB_GUI,"%s:%d: Failed to get sounders array\n", __func__, __LINE__);
        return;
    }
    
    for (i = 0; i < (unsigned int)cJSON_GetArraySize(sounder_arr_obj); i++) {
    ...
    if (found == false) {
        wifi_util_error_print(WIFI_WEB_GUI,"%s:%d: Failed to get sounder: %s\n", __func__, __LINE__, str);
        return;
    }
...
int csimgr_t::handle_result_object(cJSON *obj)
{
    ...
    if ((data_arr_obj = cJSON_GetObjectItem(obj, "data")) == NULL) {
        wifi_util_error_print(WIFI_WEB_GUI,"%s:%d: Could not find data array"
            " in result object\n", __func__, __LINE__);
        return -1;
    }
    ...
    for (i = 0; i < (unsigned int)cJSON_GetArraySize(data_arr_obj); i++) {
    ...
    if (size > 0) {
        ...
        wifi_util_info_print(WIFI_WEB_GUI,"write bytes:%d\r\n", ret);
        free(out);
    }
...
int csimgr_t::read_gesture_object(cJSON *obj)
{
    ...
    if (obj == NULL) {
        wifi_util_error_print(WIFI_WEB_GUI,"%s:%d: Invalid gesture object\n", __func__, __LINE__);
        return -1;
    }
    
    arr_obj = obj;
    
    for (i = 0; i < (unsigned int)cJSON_GetArraySize(arr_obj); i++) {
        wifi_util_info_print(WIFI_WEB_GUI,"%s:%d: Motion Descriptor: %s\n", __func__, __LINE__,
            cJSON_GetStringValue(cJSON_GetObjectItem(cJSON_GetArrayItem(arr_obj, i), "Descriptor")));
    }
...
int csimgr_t::read_capture_object(cJSON *obj)
{
    ...
    if (obj == NULL) {
        wifi_util_error_print(WIFI_WEB_GUI,"%s:%d: Invalid test object\n", __func__, __LINE__);
        return -1;
    }
...
void csimgr_t::periodicity_handler(struct timespec **t_wait)
{
    ...
    while (sd != NULL) {
        //sd->push(sd->run_test());//@TODO TBD ANIKET
...
int csimgr_t::run()
{
    ...
    if (init() != 0) {
        wifi_util_error_print(WIFI_WEB_GUI,"%s:%d: Failed to initialize\n", __func__, __LINE__);
        return -1;
    }
...
static void* csimgr_thread_entry(void* arg)
{
    csimgr_t* mgr = static_cast<csimgr_t*>(arg);
...
extern "C" int init_gui_csi_mgr_param(web_gui_obj_t *p_web_mgr)
{
    pthread_t tid;
    pthread_attr_t attr;

    p_web_mgr->gui_csi_mgr = new csimgr_t(WEB_SERVER_PATH);

    pthread_attr_init(&attr);

    if (pthread_create(&tid, &attr, csimgr_thread_entry,
                       p_web_mgr->gui_csi_mgr) != 0)
    {
        wifi_util_error_print(WIFI_WEB_GUI,"Failed to create"
            " csimgr thread\n");
        return -1;
    }
    ...
    wifi_util_info_print(WIFI_WEB_GUI,"csimgr thread started\n");
...
{
    web_gui_obj_t *p_web_mgr = get_web_gui_obj();

    if (p_web_mgr && p_web_mgr->gui_csi_mgr) {
        p_web_mgr->gui_csi_mgr->push(evt);
    }
}
