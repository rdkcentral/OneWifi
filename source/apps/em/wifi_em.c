...
static int ap_report_push_cb(em_ap_report_callback_arg_t *args)
{
    ...
    for (int i = 0; i < req_radio_count; i++) {
        ...
        ap_metrics_report->rad_reports[i].radio_index = radio_index;
        ...
        for (j = 0; j < radio->vaps.num_vaps; j++) {
            ...
            vap_report = &data->u.decoded.em_ap_metrics_report.rad_reports[i].vap_reports[j];
        ...
        rad_met = &data->u.decoded.em_ap_metrics_report.rad_reports[i].radio_metrics;
    ...
    if (data != NULL) {
        for (int j = 0; j < req_radio_count; j++) {
            for (int i = 0; i < radio->vaps.num_vaps; i++) {
                vap_report = &data->u.decoded.em_ap_metrics_report.rad_reports[j].vap_reports[i];
    ...
    wifi_mon_stats_request_state_t state, em_config_t *em_config, em_policy_req_type_t policy_type)
{
    ...
    for (i = 0; i < radio_count; i++) {
        ...
        wifi_util_error_print(WIFI_EM, "%s:%d scheduler_update_timer_task_interval else for timer task:%d with interval=%d for radio %d\n",
                    __func__, __LINE__, em_ap_metrics_report_cache.args.sched_id,
                    em_config->ap_metric_policy.interval, radio_index);
