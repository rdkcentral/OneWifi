...
#define EM_BIT_7                     (1U << 7)

#define EM_BSS_COLOR_PRESENT_BIT     EM_BIT_7
#define EM_SCAN_TYPE_ACTIVE          EM_BIT_7
    ...
    channel_scan_response_t *scan_response)
{
    ...
    for (int i = 0; i < scan_count; i++) {
	    ...
	    /*
	     * Populate channel utlization, Station count
	     * only if the bss load element is present.
	     */
            if (src->bss_load_element_present) {
                neighbor->bss_color |= EM_BSS_COLOR_PRESENT_BIT;
